...
```powershell
python -m deploy.model_package --kind humor --model-dir models/humor_transformer --output humor-model.zip
```

Keep the printed `MODEL_ARCHIVE_SHA256` and the adjacent `.sha256` file. The model package must have `config.json`, `tokenizer_config.json`, `tokenizer.json` (or `vocab.json` plus `merges.txt`), `model.safetensors`, and `thresholds.json with numeric low_threshold and high_threshold`. Review that the folder is the intended active release before packaging: the checksum verifies identity/integrity, not model quality.
...
Connect `danangnu/humor-detection-app` to Render. Create a Blueprint from this deployment branch (or merge first and use main). Review the proposed paid 2 CPU / 4 GB service and 10 GB disk before creating resources; this is initial demo sizing, not a load-tested capacity claim.
